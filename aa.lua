...
client.color_log(235, 221, 21,  "                    Last Updated: 05/03/2022.               ")
client.color_log(255, 255, 255, "|--------------------------------------------------------|")
-- our menu elements :D
...
local function notify(buffer)
    ...
    local msg = string.format("[KRIPSI AA] missed %s due to %s (hc=%d;hb=%s;fl=%s) [th=%s;td=%s]", buffer.name, buffer.reason, buffer.hitchance, buffer.hitbox, flags, buffer.target_hitbox, buffer.target_dmg)
    if buffer.type == "hit" then
        msg = string.format("[KRIPSI AA] hit %s (%d) in %s (hc=%d;fl=%s) [th=%s;td=%s]", buffer.name, buffer.damage, buffer.hitbox, buffer.hitchance, flags, buffer.target_hitbox, buffer.target_dmg)
    end
...
local function process(data)
    ...
    if data.reason then -- miss
        buffer.name = entity.get_player_name(data.target)
        buffer.reason = data.reason == "?" and "unknown" or data.reason
...
local function display()
    ...
    local miss_unknown = process_query(buffer, "reason", "unknown")
    ...
    client.color_log(10, 240, 10, string.format("Total hits: %d (%.1f%s)", total_hits, (total_shots - total_misses) / total_shots * 100, "%"))
    client.color_log(240, 10, 10, string.format("Total misses: %d (%.1f%s)", total_misses, (total_shots - total_hits) / total_shots * 100, "%"))
    client.color_log(140, 140, 140, string.format("Spread: %d; Prediction: %d; Death: %d; Unknown: %d; Unregistered shot: %d", miss_spread, miss_prediction, miss_death, miss_unknown, miss_unreg))
end
...
local function main()
    client.set_event_callback("aim_fire", function (data) 
        cache = data
